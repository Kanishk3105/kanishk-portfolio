import { motion } from "framer-motion";
import { useEffect, useState } from "react";
import { FaGithub, FaLinkedin } from "react-icons/fa";

import useParallax from "../reactbits/hooks/useParallax";
import { styles } from "../styles";
import useMediaQuery from "../utils/useMediaQuery";
import { ComputersCanvas } from "./canvas";

const Hero = () => {
  const [typedText, setTypedText] = useState("");
  const typedItems = [
    "AI/ML Engineer",
    "Python Developer",
    "Backend Developer",
    "Computer Vision Enthusiast",
    "Machine Learning Engineer",
  ];
  const [itemIndex, setItemIndex] = useState(0);
  const [charIndex, setCharIndex] = useState(0);
  const [isTyping, setIsTyping] = useState(true);

  const isMobile = useMediaQuery("(max-width: 768px)");
  const { style: parallaxStyle } = useParallax({
    strength: 0.03,
    maxOffset: 15,
    enabled: !isMobile,
  });

  useEffect(() => {
    const typeItem = () => {
      if (charIndex < typedItems[itemIndex].length) {
        setTypedText((prevText) => prevText + typedItems[itemIndex][charIndex]);
        setCharIndex(charIndex + 1);
      } else {
        setIsTyping(false);
        setTimeout(() => {
          setIsTyping(true);
          setItemIndex((itemIndex + 1) % typedItems.length);
          setCharIndex(0);
          setTypedText("");
        }, 1000); // Delay before typing the next item
      }
    };

    const typingInterval = setInterval(typeItem, 100); // Typing speed

    return () => clearInterval(typingInterval);
  }, [charIndex, itemIndex]);
  return (
    <section className={`relative w-full h-screen mx-auto`} id="hero">
      <div
        className={`absolute inset-0 top-[120px]  max-w-7xl mx-auto ${styles.paddingX} flex flex-row items-start gap-5`}
      >
        <div className="flex flex-col justify-center items-center mt-5">
          <div className="w-5 h-5 rounded-full bg-[#915EFF]" />
          <div className="w-1 sm:h-80 h-40 violet-gradient" />
        </div>

        <div style={parallaxStyle} className="relative z-10">
          <h1 className={`${styles.heroHeadText} text-white`}>
            Hi, I'm <span className="text-[#915EFF]">Kanishk Chhachra</span>
          </h1>

          <p className="text-gray-300 mt-3 text-lg">
            B.Tech CSE (AI/ML) Graduate | NVIDIA AI/ML Training Program
          </p>

          <p className={`${styles.heroSubText} mt-2 text-white-100`}>
            I'm{" "}
            <span
              className="typed"
              style={{
                backgroundImage:
                  "linear-gradient(to bottom, rgba(245,202,153,0.5), rgba(245,202,153,0.5))",
                backgroundRepeat: "no-repeat",
                backgroundSize: "100% 8px",
                backgroundPosition: "0 100%",
                color: "#915EFF",
                fontWeight: "bold",
              }}
            >
              {typedText}
            </span>
            <span>|</span>
            <br />
            <b>Building Intelligent Solutions for Real-World Problems</b>
          </p>

          {/* Social Icons */}
          <div className="flex gap-5 mt-8 relative z-[999]">
            <motion.a
              href="https://github.com/Kanishk3105"
              target="_blank"
              rel="noopener noreferrer"
              whileHover={{ scale: 1.15, y: -5 }}
              whileTap={{ scale: 0.95 }}
              animate={{ y: [0, -6, 0] }}
              transition={{
                duration: 2,
                repeat: Infinity,
                ease: "easeInOut",
              }}
              className="
        w-14 h-14
        rounded-full
        flex items-center justify-center
        bg-[#121826]
        border border-[#915EFF]
        shadow-[0_0_20px_rgba(145,94,255,0.5)]
        hover:shadow-[0_0_35px_rgba(145,94,255,0.9)]
        cursor-pointer
      "
            >
              <FaGithub size={26} className="text-white" />
            </motion.a>

            <motion.a
              href="https://www.linkedin.com/in/kanishk-chhachra-654171276/"
              target="_blank"
              rel="noopener noreferrer"
              whileHover={{ scale: 1.15, y: -5 }}
              whileTap={{ scale: 0.95 }}
              animate={{ y: [0, -6, 0] }}
              transition={{
                duration: 2,
                repeat: Infinity,
                ease: "easeInOut",
                delay: 0.4,
              }}
              className="
        w-14 h-14
        rounded-full
        flex items-center justify-center
        bg-[#121826]
        border border-[#00D4FF]
        shadow-[0_0_20px_rgba(0,212,255,0.5)]
        hover:shadow-[0_0_35px_rgba(0,212,255,0.9)]
        cursor-pointer
      "
            >
              <FaLinkedin size={26} className="text-white" />
            </motion.a>
          </div>
        </div>
      </div>

      <div className="absolute inset-0 z-0">
        <ComputersCanvas />
      </div>

      <div className="absolute xs:bottom-10 bottom-32 w-full flex justify-center items-center">
        <a href="#about">
          <div className="w-[35px] h-[64px] rounded-3xl border-4 border-secondary flex justify-center items-start p-2">
            <motion.div
              animate={{
                y: [0, 24, 0],
              }}
              transition={{
                duration: 1.5,
                repeat: Infinity,
                repeatType: "loop",
              }}
              className="w-3 h-3 rounded-full bg-secondary mb-1"
            />
          </div>
        </a>
      </div>
    </section>
  );
};

export default Hero;
